      ...
      const fetchData = async () => {
        try {
          ...
          const response = await axios.get(
            `${ApiUrl}/api/dashboard/counts?currDate=${currentDate}`
          );
          setData(response.data);

          // Fetch pending payment data (example API for pending payment)
          const pendingPaymentResponse = await axios.get(
            `${ApiUrl}/api/dashboard/pendingBills`
          );
          const paymentsData = pendingPaymentResponse.data;

          // Calculate total pending amount
          const totalAmount = paymentsData.reduce((total, item) => {
            const amount = parseFloat(
              item.pendingPayment.replace("₹", "").replace(",", "")
